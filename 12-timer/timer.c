...
***************************************************************/
#if 1
#define print_debug(fmt, args...) printk("【 DEBUG 】"fmt"\n", ##args)
#else
...
static int led_init(void)
{
	...
	/* 设置LED所使用的GPIO */
	/* 1、获取设备节点：timerdev */
	timerdev.nd = of_find_node_by_path("/led");
	if(timerdev.nd == NULL) {
	...
	/* 2.读取status属性 */
	ret = of_property_read_string(timerdev.nd, "default-state", &str);
	if(ret < 0) 
	    return -EINVAL;
...
static int timer_open(struct inode *inode, struct file *filp)
{
    ...
    print_debug("open timer cdev start");
	timerdev.timeperiod = 1000;		/* 默认周期为1s */
	ret = led_init();				/* 初始化LED IO */
    print_debug("open timer cdev end");
	if (ret < 0) {
		return ret;
	}
...
static int led_release(struct inode *inode, struct file *filp)
{
	...
	gpio_set_value(dev->led_gpio, 0);	/* APP结束的时候关闭LED */
