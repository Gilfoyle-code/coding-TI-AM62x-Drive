...
其他	   	: 无
论坛 	   	: www.openedv.com
日志	   	: 初版V1.0 2023/03/02 正点原子Linux团队创建
***************************************************************/
...
static int led_gpio_init(struct device_node *nd)
{
	...
	/* 从设备树中获取GPIO */
	leddev.gpio_led = of_get_named_gpio(nd, "gpios", 0);
	if(!gpio_is_valid(leddev.gpio_led)) {
        printk(KERN_ERR "leddev: Failed to get gpios\n");
        return -EINVAL;
    }
...
static struct platform_driver led_driver = {
	.driver		= {
		.name	= "alientek,led",			/* 驱动名字，用于和设备匹配 */
