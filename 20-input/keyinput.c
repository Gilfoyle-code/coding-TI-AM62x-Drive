...
其他	   	: 无
论坛 	   	: www.openedv.com
日志	   	: 初版V1.0 2021/01/30 正点原子Linux团队创建
***************************************************************/
...
static int key_gpio_init(struct device_node *nd)
{
	...
	/* 从设备树中获取GPIO */
	key.gpio_key = of_get_named_gpio(nd, "gpios", 0);
	if(!gpio_is_valid(key.gpio_key)) {
		printk("key: Failed to get gpios\n");
		return -EINVAL;
	}
	...
	printk("ret = %#X\r\n", ret);
    if (ret) {
        printk(KERN_ERR "key: Failed to request gpios\n");
	...
	key.irq_key=of_irq_get(nd, 0);
	
	/* 获取GPIO对应的中断号 */
	key.irq_key = irq_of_parse_and_map(nd, 0);
	if (key.irq_key == NO_IRQ) {
    	printk("No interrupt found in the device tree\n");
    	return -EINVAL;
	}
...
static int atk_key_probe(struct platform_device *pdev)
{
	int ret;
	if (pdev->dev.of_node) {  
   		printk(KERN_INFO "Found node with path: %s\n", pdev->dev.of_node->full_name);  
    	// 如果需要打印更多的属性信息，可以这样做：  
    	const char *compatible = of_get_property(pdev->dev.of_node, "compatible", NULL);  
    	if (compatible) {  
        	printk(KERN_INFO "Compatible string: %s\n", compatible);  
    	}  

		const char *pinctrl_name = of_get_property(pdev->dev.of_node, "pinctrl-name", NULL);  
    	if (pinctrl_name) {  
        	printk(KERN_INFO "pinctrl_name string: %s\n", pinctrl_name);  
    	}  
